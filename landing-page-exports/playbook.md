[//]: # (URL: https://ianryankirk.com/playbook)

# Page Metadata

**Page Type:** Lead Magnet / Free Playbook (DownloadGate — email capture unlocks downloadable HTML)
**Page Title:** The Facebook Playbook
**Meta Description:** (not explicitly set — inferred) The exact 5-step system a multiple 7-figure operator walked Ian through in Cabo. He built a 24,000-member group and did $1.1M in a single day from it.

---

# Hero

**Label Tag:** Free Playbook · Facebook Platform Strategy

## The Facebook
### Playbook

The exact 5-step system a multiple 7-figure operator walked me through in Cabo. He built a 24,000-member group and did $1.1M in a single day from it.

**CTA Buttons:**
- [Read the Playbook →](#the-playbook) — anchors to playbook section
- [Download the PDF](#) — triggers DownloadGate email capture modal

### Social Proof Bar

Built a 24,000-member Facebook Group. $1.1M in a single day. Sold the whole thing in a multi-million dollar exit.

*Here's what he showed me.*

---

# The Playbook

## 01 The 3-Post Bank

Posting daily sounds like a grind until you realize you can batch-create three posts in one sitting and have them ready to fire from a Google Doc.

The creators who show up every day aren't more inspired. They're more prepared.

Three posts in your back pocket means you never miss a day because you don't know what to write.

> *"Consistency beats perfection on Facebook."*

---

## 02 The Comment Stack Format

Stop writing essays in the post body. Hook. Numbered promise. Period.

Then drop each step as its own comment right after you post. More total reactions. More dwell time. More algorithmic reach. You create a thread people scroll through instead of scroll past.

The hook earns the click. The comments deliver the value. That's the whole game.

> *"The hook earns the click. The comments deliver the value."*

---

## 03 The 5,000 Friends Rule

Your Facebook friends list isn't a social network. It's a warm list you haven't activated yet.

Quality matters more than size. Friends from your niche, from events, from communities you're already in are worth 10x a random add.

You don't need a massive audience to make real money on Facebook. You need the right 1,000 people paying attention.

> *"You don't need a massive audience. You need the right 1,000 people paying attention."*

---

## 04 The Algorithm Shift That Changed Everything

Facebook quietly rewired how it ranks content. It's not about likes anymore. It's not about reach.

It's about comment activity. What's happening inside the thread.

Structure your content so the value lives in the comments. Every step you add, every follow-up you drop, the algorithm reads it as engagement and pushes your content to more people.

> *"Structure beats effort. Every time."*

---

## 05 The Platform Nobody's Fighting Over

Everyone in B2B got the memo that LinkedIn is "the professional network." So they all piled in. Cost per eyeball went up. Noise went up. Results went down.

Facebook users spend 2 hours a day on the platform. LinkedIn? 15 minutes. And the audience skews older, which usually means more money, more buying authority, and decisions that have already been made.

> *"The best opportunity is always where everyone else stopped looking."*

---

# More Free Tools

## While You're Here

These tools are free. Use them on your own or bring them into a call and we'll work through them together.

### Business Clarity Coach
Stuck between ideas and zero traction? This locks in your one clear path forward in one session.
**CTA:** [Get Clarity →](https://ultimate-clarity-coach.vercel.app/) — external link

### Call Reflekt
Paste a sales call transcript. Get back exactly where you lost the frame.
**CTA:** [Analyze a Call →](https://call-reflekt-coach.vercel.app/) — external link

### DM Copywriter (SLAP Method)
Turn ignored DMs into booked calls. The exact framework Ian uses.
**CTA:** [Try It Free →](https://slap-method.vercel.app/) — external link

### Cold Email Copywriter
Write cold emails that get replies. Poke The Bear framework, reply-focused CTAs, no AI fluff.
**CTA:** [Write Cold Emails →](https://cold-email-copywriter-opal.vercel.app) — external link

---

# Take It With You

## Download the
### Full Playbook.

All 5 steps as a printable PDF. Open it in your browser and hit Ctrl+P to save.

**Primary CTA:** [Download the Facebook Playbook →](#) — triggers DownloadGate email capture modal
**Secondary CTA:** [Book a Clarity Call →](https://www.leadgenjay.com/book-ian) — external link (LeadGenJay booking page)

---

## Internal Links Found On This Page

| Anchor Text | Destination URL |
|-------------|-----------------|
| *(No internal ianryankirk.com links — all CTAs are external or DownloadGate)* | — |
